...
| 文件 | 内容 |
|---|---|
| `companies.js` | 公司库：加公司就是往 `companies` 数组里加一个对象；`match` 字段是对你背景的匹配度 1-3；`graduationWindow` 是毕业时间窗口（延毕风险）板块的数据源 |
...
> 注意：这里的 39 家是「有岗位的公司」；作战室企业赛道的完整公司库在 `src/data/companies.js`（89 家，其中科研机构 15 家），两者通过 `src/data/jobConfig.js` 的 `COMPANY_ALIAS` 做 id 映射（如雷达侧 `iie` / `ict` ↔ 作战室 `cas-iie` / `cas-ict`）。
...
### 2026-09-16 第二轮核实：新增 5 家公司 / 15 条岗位

9 月中旬复查发现以下渠道已开放，岗位数 **131 → 146**，公司库 **25 → 30 家**：

| 公司 | 核实结果 | 新增岗位 |
|---|---|---|
| 阿里巴巴（淘天集团） | 2026-08-05 启动网申至 11 月，AI 技术类岗位占比超九成，截止 2026-11-30；每人 2 个志愿、仅 1 次投递机会 | 4 |
| 中国移动（集团 / 专业公司） | 2027 校招全面启动：中移九天（AI 开发 / 算法开发，面向 2027 届硕博）、移动云（AI Infra / 大模型 / 推理 / Agent + 金种子计划） | 3 |
| 上海人工智能实验室 | 2027 届走「留用实习生」通道（毕业时间 2026.10–2027.9），官网校招页 100+ 职位 | 2 |
| 鹏城国家实验室 | 研究岗面向应（往）届博士长期开放（hr.pcl.ac.cn），研究员 120 万起 | 1 |
| 之江实验室 | 招聘官网上线，校园招聘 50 个在招职位 + 博士后 31 个（杭州） | 1 |
| 智源研究院（BAAI） | 27 秋招正式启动、截止 2026-09-26；新增 AI 编译器、具身大模型、**AI 安全-网络对抗** 三个具体方向 | 3（另给原记录补上截止日期） |
| 商汤科技 | 2027 届岗位需求同比 +35%，新推「无限原力」顶尖人才计划；官方口径为「全年开放投递、招满即下架」→ 原 5 条截止日期改为滚动 | 1 |

**本轮仍未核实（留给下一轮）**：vivo、生数科技、第四范式、奇安信、微软 / 英伟达中国区、比亚迪、米哈游 / 鹰角、浪潮、海尔 / 海信 / 格力。

### 2026-09-16 第三轮核实：科研机构 / 实验室专项，新增 9 家机构 / 15 条岗位

针对「像上海 AI Lab 这样的实验室型研究机构」做了一轮专项检索。岗位数 **146 → 161**，公司库（`jobCompanies.json`）**30 → 39 家**；作战室公司库（`companies.js`）同步新增 6 家科研机构条目，科研机构由 9 家增至 **15 家**。

| 机构 | 性质 | 本轮核实要点 | 新增岗位 |
|---|---|---|---|
| 上海人工智能实验室 | 新型研发机构 | 2026-08-28 一次集中发布 5 个安全岗：大模型内生安全青年科学家/研究员、智能体攻防算法工程师、安全算法工程师/青年研究员、基模算法青年研究员、AI 前沿风险青年研究员 | 2 |
| 北京智源研究院 | 非营利新型研发机构 | 「智星」计划九大方向含 **AI 安全**（可信 AI 安全研究组：红蓝对抗 / 安全边界认知 / 对齐）；截止 2026-09-26 | 0（原有 3 条，本轮补全方向描述） |
| 中国信通院 | 工信部直属科研事业单位 | 27 届人工智能研究所网申 2026-08-07–10-07；「智定者」计划给事业单位正式编制 + 100% 北京户口 | 2 |
| 中关村实验室 | 中央管理的国家科研事业单位 | 27 届要求 **2027-12-31 前取得双证**（本清单毕业窗口最宽），投递截止 2026-11-30，邮箱投递 | 1 |
| 中科院计算技术研究所 | 中科院事业单位 | 智能算法安全全国重点实验室；特别研究助理 A 类年薪 30 万起、入选资助项目 ≥80 万经费、可转事业编 | 1 |
| 中科院信息工程研究所 | 中科院事业单位 | 网络空间安全防御全国重点实验室（信息对抗 / 智能认知安全 / 数据安全研究室）；2026-07 举办「大模型隐私保护」学术交流会 | 1 |
| 紫金山实验室 | 江苏省市共建事业单位 | 普适通信（6G 新型无线传输 / 6G 网络 AI）+ 网络内生安全（邬江兴院士团队）两条落点 | 2 |
| 启元实验室 | 中央管理的新型科研事业单位 | 27 届校招 5 类共 78 人；软件开发类明确含「模型安全 / 代码安全 / 形式化验证」 | 1 |
| 上海期智研究院 | 上海市科委事业单位 | 姚期智院士牵头（2020 揭牌），人工智能 + 现代密码学 + 高性能计算 + 量子计算，28 个研究课题 | 1 |
| 上海人工智能研究院 | 上海市与上海交大共建 | **与「上海人工智能实验室」是两个独立主体**，勿混投；邮箱投递 | 1 |
| 合肥国家实验室 | 国家实验室 | 以量子信息为主，设密码安全 / 密码检测岗；技术支撑岗常年招聘（不限应届） | 1 |
| 中国移动研究院 | 国企研发机构 | JD 原文明确写「**语义通信**」；同批次另有「人工智能安全」博士岗；截止 2026-10-17 | 2 |

**本轮顺带修掉的两个问题：**

1. **坏链接（18 条）**：`companies.js` 的 `url` 字段里，部分条目把提示语直接跟在地址后面，如 `https://www.caict.ac.cn（关注"人才招聘"栏）`。`Enterprise.vue` 用 `c.url.startsWith('http')` 判断是否渲染成链接，这类脏值全部会被渲染成**点不开的 404 链接**。已两手处理：① 数据侧修正 4 条（信通院、信工所、自动化所、格力）并把提示文字移入 `note`；② 渲染侧新增 `cleanUrl()`，统一截取到第一个非 ASCII 字符，一次覆盖剩余 14 条及未来新增。
2. **id 映射缺失**：雷达侧中科院所属研究所用短 id，作战室公司库沿用 `cas-` 前缀，导致新机构岗位无法从公司库跳转岗位雷达。已在 `jobConfig.js` 的 `COMPANY_ALIAS` 补 `iie → cas-iie`、`ict → cas-ict`。

### ⏰ 毕业时间窗口（延毕风险）—— 重要

校招岗位卡的**不是「哪一年毕业」，而是「是否在某个日期前取得毕业证和学位证」**；这个日期多数是毕业次年 **8 月 31 日**，少数放宽到当年 **12 月 31 日**。本人理论毕业时间为 2027 年 6 月，但硬性毕业条件尚未达成、存在延毕可能，因此本轮把各机构的最晚拿证日单独核实，落在 `companies.js` 的 `graduationWindow` 导出中，作战室「企业赛道」页已渲染为独立板块。

| 机构 | 最晚拿证窗口 | 宽严 |
|---|---|---|
| 北京智源研究院 | 2026-09-01 ~ 2027-08-31（另要求毕业后无全职工作经历） | 最严 |
| 中国信通院 | 2026-09 ~ 2027-08 | 严 |
| 上海人工智能实验室 | 2026-10 ~ 2027-09 | 严 |
| 携程 / 商汤科技 | 2026-09 ~ 2027-08 / 2026-09 ~ 2027-12 | 中 |
| 蚂蚁集团 | 2026-11 ~ 2027-10 | 宽 |
| 中关村实验室 | 2027-12-31 前取得双证 | 最宽 |
| 华为 | 2026-01-01 ~ 2027-12-31 | 最宽 |
| 紫金山 / 鹏城 / 之江 / 启元 / 中科院各所 / 上海期智 | 无应届毕业时间硬约束（走常年社招或博士后通道） | 兜底 |

**结论：**

1. **2027 年 6 月正常毕业** → 上表全部可选，按截止日先后投（智源 9/26 → 信通院 10/7 → 移动研究院 10/17 → 中关村 11/30）。
2. **延毕到 2027 年 9-12 月** → 「次年 8 月 31 日」那一档的应届通道已关闭，只能等 2028 届；但中关村实验室、华为、商汤（放宽至 2027-12-31）仍可投。
3. **延毕到 2028 年** → 校招应届身份整体失效，转走博士后岗与社招岗。**博士后只看获学位年限（一般 ≤3 年），不看应届身份**，紫金山 / 启元 / 中关村均设站，中科院各所与紫金山 / 鹏城 / 之江常年招社招。
4. 一句话：**延毕不会让你断档，但会换赛道**——从「校招应届生」换成「博士后 / 社招」。所以毕业时间一旦有变，第一件事是重算「最晚拿证日期」，而不是重投简历。

**本轮仍未核实（留给下一轮）**：IDEA 研究院（深圳）、上海算法创新研究院、北京科学智能研究院、中国电信 / 联通研究院的 27 届批次。

### 下一轮待补充（2026-08-28 原始清单）

> 2026-09-16 更新：上海人工智能实验室、鹏城国家实验室、之江实验室 三家的 2027 届通道均已开放并已收录（见上一节）；微软 / 英伟达、vivo、生数科技、第四范式 等仍待跟进。
...
- 中国移动：招聘站可访问且已开「人工智能专区（校招）」`job.10086.cn/personal/job/?code=115&zpcode=01` 与「算力专区」（code=117），**当时 0 岗**，27 届公告预计 8 月底-9 月放出，值得盯 —— **2026-09-16 更新：已全面启动，已收录 3 条岗位（中移九天 / 移动云）**
...
- 阿里巴巴 / 淘宝闪购：校招统一走 `talent.alibaba.com`，当时应届生批次列表未放出（显示 0 岗），淘系岗位 9 月随集团秋招一起放 —— **2026-09-16 更新：淘天集团已于 2026-08-05 启动 2027 届校招，已收录 4 条岗位**
...
作战室的公务员 / 选调时间节点核实于 2026-08-29（见「官方信息入口」）；岗位雷达数据最近一轮核实于 **2026-09-16**（第三轮：科研机构专项，新增 9 家机构 / 15 条岗位；第二轮同日新增 5 家公司 / 15 条岗位），逐条核实日期见 `jobs.json` 的 `last_verified_at`。投递前务必以各公司官网为准。

> 科研机构类的招聘窗口普遍只有 2–6 周，且公告易撤回、部分机构只有邮箱入口没有公开官网（如中关村实验室）。这类条目投递前必须重新打开官方入口核对一次。
